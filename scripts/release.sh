...
build_tarball() {
  ...
  # src/mcapp/ package -- CODE AND PACKAGE DATA.
  #
  # The `-o -name '*.json'` is load-bearing, not tidiness. This used to copy
  # `*.py` only, which shipped every *_tests.py module while silently dropping
  # all eleven .json files they read by path: the five hand-copied vector
  # corpora and the three wire contracts under contract/. Every deployed slot
  # on mcapp.local carried zero .json under src/mcapp, and src/mcapp/contract/
  # did not exist at all (found at v2.0.11). Nothing breaks at RUNTIME -- no
  # production code path reads a packaged .json (config.json comes from
  # /etc/mcapp, load_dump takes an admin-supplied path) -- but the shipped tree
  # did not match the source tree, and a suite run on the box failed on missing
  # data rather than on anything real. Pinned by webapp_deploy_tests.py.
  mkdir -p "${staging}/src/mcapp/commands"
  find "${PROJECT_DIR}/src/mcapp" \( -name '*.py' -o -name '*.json' \) \
  ...
  # Same code-and-data predicate as src/mcapp above. ble_service is pure Python
  # today; keeping the two identical means adding a data file there later does
  # not silently repeat the bug this predicate exists to fix.
  find "${PROJECT_DIR}/ble_service/src" \( -name '*.py' -o -name '*.json' \) \
    -not -path '*/__pycache__/*' | while read -r f; do
    local rel="${f#${PROJECT_DIR}/}"
    mkdir -p "${staging}/$(dirname "$rel")"
    cp "$f" "${staging}/${rel}"

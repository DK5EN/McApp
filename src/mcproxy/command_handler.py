...
class CommandHandler:
    ...
    def _check_test_completion(self, test_id: str) -> bool:
        ...
        if total_completed > expected_total:
            if has_console:
                print(
                    f"⚠️ Test {test_id} over-completion detected: {total_completed}/{expected_total}"
                )
    ...
    def _cleanup_throttle_cache(self, current_time, timeout=None):
        ...
        expired = [
            chash for chash, timestamp in self.command_throttle.items() if timestamp < cutoff
        ]
        ...
        for part in parts[1:]:
            if ":" in part:
                ...
                if cmd in ["s", "search"] and not kwargs:
                    ...
                    # Handle ctcping arguments:
                    # !ctcping target:OE5HWN-12 call:OE1ABC payload:25 repeat:3
    ...
    async def test_ctcping_logic(self):
        ...
        if "echo" in description.lower():
            if "Non-ping echo ignored" in description:
                # Test complete echo filtering logic -
                # extract original message and test if it's a ping
                original_msg = (
                    message[:-4] if message.endswith("}") and len(message) >= 4 else message
                )
    ...
    async def test_reception_edge_cases(self):
        ...
        edge_cases = [
            ...
            (
                "oe1abc-5",
                self.my_callsign.lower(),
                f"!time {self.my_callsign.lower()}",
                True,
                True,
                "direct",
                "Lowercase handling",
            ...
            (
                "OE1ABC-5",
                "20",
                f"!wx OE1ABC-5 {self.my_callsign}",
                True,
                True,
                "group",
    ...
    async def test_kickban_logic(self):
        ...
        test_cases = [
            ...
            #  expected_result_contains, expected_blocked_after, description)
            # === Admin Tests ===
            (
                self.admin_callsign_base,
                {},
            ...
            (
                self.admin_callsign_base,
                {"callsign": f"{self.admin_callsign_base}-99"},
                set(),
                "❌ Cannot block own callsign",
                set(),
        ...
        for src, dst, msg, groups_enabled, expected_exec, expected_type, description in test_cases:
            ...
            try:
                ...
                if has_console:
                    ...
                    target = (
                        self.extract_target_callsign(msg)
                        if hasattr(self, "extract_target_callsign")
                        else None
                    )
                    intent = (
                        "LOCAL"
                        if is_our_msg and (not target or target == self.my_callsign)
                        else "REMOTE"
                        if is_our_msg
                        else "N/A"
                    )
    ...
    async def test_self_command_execution(self):
        ...
        test_cases = [
            ...
            # ("!ctcping target:DK5EN-1 call:DK5EN-99 payload:80 repeat:1",
            #  ["🏓", "Ping test", "DK5EN-99", "started"],
            #  "CTC Ping should start ping test"),
            # ("!ctcping call:DK5EN-99 payload:20 repeat:2",
            #  ["🏓", "Ping test", "DK5EN-99", "started"],
            #  "CTC Ping should start ping test"),
            # ("!ctcping call:DK5EN-99 payload:120",
            #  ["🏓", "Ping test", "DK5EN-99", "started"],
            #  "CTC Ping should start ping test"),
            # ("!ctcping call:DK5EN-99",
            #  ["🏓", "Ping test", "DK5EN-99", "started"],
            #  "CTC Ping should start ping test"),
            # Help and info
    ...
    async def test_remote_command_execution(self):
        ...
        test_cases = [
            ...
            (
                ...
                True,
                "local",
                "Test group command without target get executed"
                " locally and result is sent to group",
            ),
    ...
    async def test_self_command_suppression_logic(self):
        ...
        for command, description in test_cases:
            try:
                ...
                test_data = {"src": self.my_callsign, "dst": self.my_callsign, "msg": command}
    ...
    async def test_topic_logic(self):
        ...
        try:
            # Set up test beacons
            await self.handle_topic(
                {"group": "50", "text": "Test beacon 1", "interval": 60}, self.admin_callsign_base
            )
            await self.handle_topic(
                {"group": "51", "text": "Test beacon 2", "interval": 120}, self.admin_callsign_base
            )
    ...
    async def test_incoming_personal_commands(self):
        ...
        # Test cases: (src, dst, command, should_execute,
        #  expected_type, expected_response_dst, description)
        test_cases = [
            ...
            (
                "DK5EN-99",
                self.my_callsign,
                f"!SEARCH CALL:DK5EN-1 {self.my_callsign}",
                True,
                "direct",
                "DK5EN-99",
            ...
            (
                "DK5EN-99",
                self.my_callsign,
                f"!POS CALL:DB0ED-99 {self.my_callsign}",
                True,
                "direct",
                "DK5EN-99",
            ...
            (
                "DK5EN-99",
                self.my_callsign,
                f"!MHEARD LIMIT:5 {self.my_callsign}",
                True,
                "direct",
                "DK5EN-99",
            ...
            (
                "DK5EN-99",
                self.my_callsign,
                f"!CTCPING TARGET:{self.my_callsign} CALL:W1XYZ-1",
                True,
                "direct",
                "DK5EN-99",
            ...
            (
                "DK5EN-99",
                self.my_callsign,
                f"!CTCPING CALL:DK5EN-99 {self.my_callsign}",
                True,
                "direct",
                "DK5EN-99",
    ...
    async def test_incoming_personal_commands_old(self):
        ...
        test_cases = [
            ...
            (
                ...
                False,
                None,
                "Weather request without target should not execute",
            ),
            (
                "DK5EN-99",
                self.my_callsign,
                ...
                False,
                None,
                "Time request without target should not execute",
            ),
            (
                "DK5EN-99",
                self.my_callsign,
                ...
                False,
                None,
                "Dice request without target should not execute",
            ),
            (
                "DK5EN-99",
                self.my_callsign,
            ...
            (
                ...
                True,
                "direct",
                "Our weather command to others should execute locally and send result",
            ),
            (
                self.my_callsign,
                "DK5EN-99",
                ...
                True,
                "direct",
                "Our time command with our target should execute locally and send result",
            ),
            (
                self.my_callsign,
                "OE1ABC-5",
                ...
                True,
                "direct",
                "Our dice command to others should execute locally and send result",
            ),
            (
                self.my_callsign,
                "W1XYZ-1",
                ...
                True,
                "direct",
                "Our stats command to others should execute locally and send result",
            ),
        ]

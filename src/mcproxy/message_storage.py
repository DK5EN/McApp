...
class MessageStorageHandler:
    ...
    def load_dump(self, filename):
        ...
        if os.path.exists(filename):
            with open(filename, "r", encoding="utf-8") as f:
                ...
                size_kb = self.message_store_size / 1024
                print(f"{count} Nachrichten ({size_kb:.2f} KB) geladen")
    ...
    async def get_smart_initial(self, limit_per_dst: int = 15) -> dict:
        ...
        for i in recent_items:
            ...
            if '"type": "msg"' in raw and ":ack" not in raw:
                try:
                    data = json.loads(raw)
                    dst = data.get("dst")
                    if dst is not None and len(msgs_per_dst[dst]) < limit_per_dst:
                        msgs_per_dst[dst].append(raw)
                except json.JSONDecodeError:
                    continue
            elif '"type": "pos"' in raw:
    ...
    async def get_messages_page(
        ...
        matching = []
        for item in reversed(list(self.message_store)):
            raw = item["raw"]
            if '"type": "msg"' not in raw or ":ack" in raw:
                continue
            try:
                data = json.loads(raw)
            except json.JSONDecodeError:
    ...
    def process_mheard_store(self):
        ...
        for item in self.message_store:
            ...
            if not (is_valid_value(rssi, *VALID_RSSI_RANGE)
               and is_valid_value(snr, *VALID_SNR_RANGE)):
                continue
    ...
    async def process_mheard_store_parallel(self, progress_callback=None):
        ...
        raw_buckets = await self._process_chunks_parallel(
            cutoff_timestamp_ms, progress_callback=progress_callback
        )
        if not raw_buckets:
            if progress_callback:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            try:
                ...
                if progress_callback:
                    await progress_callback(
                        "chunking",
                        f"Crunching data ({successful_chunks}/{len(chunks)} chunks)"
                    )
    ...
    def _create_segments_with_gaps(self, raw_stats):
        ...
        if has_console and filtered_callsigns:
            ...
            print(f"📊 Filtered {len(filtered_callsigns)} callsigns"
                  f" with <{MIN_DATAPOINTS_FOR_STATS} data points:")
    ...
    def _log_processing_summary(self, raw_stats, final_result):
        ...
        print(f"📊 Final result: {len(final_result)} total points"
              f" ({len(stats_entries)} stats + {len(gap_markers)} gaps)")
        print(f"📊 Callsigns: {final_callsigns} included,"
              f" {filtered_count} filtered"
              f" (min {MIN_DATAPOINTS_FOR_STATS} points)")
        ...
        if stats_entries:
            ...
            print(f"📊 Processed {unique_callsigns} callsigns"
                  f" with {len(segment_stats)} having gaps")
